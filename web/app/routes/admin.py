...
@admin_bp.route("/pedidos-online")
def pedidos_online():
    """Histórico completo de pedidos online, separado por data — não exige
    que o admin tenha um caixa aberto (o admin normalmente nunca abre
    caixa; quem abre é o operador no balcão)."""
    if not _sessao_e_admin():
        return redirect(url_for("auth.login"))
    ...
    try:
        resposta = requests.get(
            f"{API_URL}/pedidos-online/historico", headers=_headers(), timeout=5
        )
        if resposta.status_code == 200:
            pedidos = resposta.json()
        else:
            flash("Não foi possível carregar os pedidos online.", "erro")
    except requests.exceptions.RequestException:
        flash("Não foi possível conectar à API. Verifique se ela está no ar.", "erro")

    grupos_por_data = OrderedDict()
    for pedido in pedidos:
        criado_em = pedido.get("criado_em", "")
        try:
            data_obj = datetime.fromisoformat(criado_em)
            chave = data_obj.strftime("%d/%m/%Y")
        except (ValueError, TypeError):
            chave = "Sem data"
        grupos_por_data.setdefault(chave, []).append(pedido)

    return render_template(
        "admin/pedidos_online.html",
        usuario=session.get("usuario"),
        grupos_por_data=grupos_por_data,
