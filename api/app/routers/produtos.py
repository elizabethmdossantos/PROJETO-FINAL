...
@router.get("/caixa", response_model=list[ProdutoOut])
def listar_produtos_para_caixa(
    busca: str = "",
    skip: int = 0,
    ...
    """Lista de produtos cadastrados para o menu lateral do terminal de
    venda — qualquer usuário autenticado (operador de caixa ou admin) pode
    consultar, ao contrário de `GET /produtos`, que é restrito ao admin.
    O operador usa isso para não precisar decorar códigos de produto."""
    consulta = db.query(Produto).filter(Produto.ativo.is_(True))
    if busca:
        consulta = consulta.filter(
            (Produto.nome.ilike(f"%{busca}%")) | (Produto.codigo.ilike(f"%{busca}%"))
        )
