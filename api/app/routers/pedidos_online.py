...
def listar_pedidos_online(
    status_filtro: Optional[StatusPedidoOnline] = StatusPedidoOnline.AGUARDANDO_RETIRADA,
    skip: int = 0,
    limit: int = Query(50, le=500),
    db: Session = Depends(get_db),
    _usuario: dict = Depends(usuario_atual),
):
    """Por padrão retorna só os pedidos aguardando retirada (fila do
    balcão). Para o histórico completo (todos os status), veja
    `GET /pedidos-online/historico`."""
    pedidos = db.query(PedidoOnline)
    if status_filtro is not None:
        pedidos = pedidos.filter(PedidoOnline.status == status_filtro)
    pedidos = (
        pedidos.order_by(PedidoOnline.criado_em.asc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.get("/pedidos-online/historico", response_model=list[PedidoOnlineOut])
def listar_historico_pedidos_online(
    skip: int = 0,
    limit: int = Query(500, le=1000),
    db: Session = Depends(get_db),
    _usuario: dict = Depends(usuario_atual),
):
    """Histórico completo — todos os status, mais recentes primeiro. Usado
    pelo painel do admin para listar os pedidos online já realizados,
    separados por data, sem qualquer exigência de caixa aberto."""
    pedidos = (
        db.query(PedidoOnline)
