...
Sistema de gestão comercial (ERP enxuto / PDV) com back-end em **FastAPI + MySQL**
...
docker compose up -d
```
Isso sobe um MySQL 8 já com o schema `pdv_db` criado, usando as variáveis
`DB_PASSWORD`, `DB_NAME` e `DB_PORT` do seu `.env` (ou os valores padrão
`pdv_root_password` / `pdv_db` / `3306` se você não definir nada).

**Opção B — MySQL Workbench / instalação local:**
Crie um schema vazio, por exemplo `pdv_db`. As tabelas são criadas
automaticamente pela API na primeira execução (`Base.metadata.create_all`).
...
├── api/                     → FastAPI + SQLAlchemy (MySQL) + JWT
├── web/                     → Flask + Jinja2, consome a API via HTTP
├── tests/                   → Pytest (cobertura da camada de API)
├── docker-compose.yml       → MySQL pronto para desenvolvimento
